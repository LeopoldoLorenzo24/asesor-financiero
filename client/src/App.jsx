...
export default function App() {
  ...
  const renderOperaciones = () => {
    const COLORS = [T.green, T.blue, T.purple, T.yellow, T.orange, T.red, T.greenLight, T.pink, "#06b6d4", "#f43f5e", "#a3e635", "#fb923c"];
    const SECTOR_COLORS = { Technology: T.blue, Healthcare: T.green, "Consumer Cyclical": T.yellow, "Consumer Defensive": T.orange, Financial: T.purple, Energy: T.red, Materials: "#a3e635", "Communication Services": T.pink, Industrials: "#06b6d4", ETF: T.greenLight };
    ...
    return (<div>
      ...
      {pieData.length > 1 && <div style={{ display: "flex", gap: 20, flexWrap: "wrap", marginTop: 20 }}>
        <div style={{ ...S.card, flex: 1, minWidth: 320 }}>
          <div style={{ ...S.label, textAlign: "center", marginBottom: 8 }}>Distribución por CEDEAR</div>
          <ResponsiveContainer width="100%" height={280}><PieChart><Pie data={pieData} cx="50%" cy="50%" innerRadius={55} outerRadius={95} dataKey="value" paddingAngle={2} label={renderLabel} labelLine={{ stroke: T.textDark, strokeWidth: 1 }} style={{ fontSize: 11, fontFamily: "JetBrains Mono, monospace", fill: T.text }}>{pieData.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} stroke={T.bg} strokeWidth={2} />)}</Pie><Tooltip contentStyle={{ background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: 8, fontSize: 12 }} formatter={v => `$${Math.round(v).toLocaleString()}`} /></PieChart></ResponsiveContainer>
        </div>
        {sectorData.length > 1 && <div style={{ ...S.card, flex: 1, minWidth: 320 }}>
          <div style={{ ...S.label, textAlign: "center", marginBottom: 8 }}>Distribución por Sector</div>
          <ResponsiveContainer width="100%" height={280}><PieChart><Pie data={sectorData} cx="50%" cy="50%" innerRadius={55} outerRadius={95} dataKey="value" paddingAngle={2} label={renderLabel} labelLine={{ stroke: T.textDark, strokeWidth: 1 }} style={{ fontSize: 11, fontFamily: "JetBrains Mono, monospace", fill: T.text }}>{sectorData.map((entry, i) => <Cell key={i} fill={SECTOR_COLORS[entry.name] || COLORS[i % COLORS.length]} stroke={T.bg} strokeWidth={2} />)}</Pie><Tooltip contentStyle={{ background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: 8, fontSize: 12 }} formatter={v => `$${Math.round(v).toLocaleString()}`} /></PieChart></ResponsiveContainer>
        </div>}
      </div>}
